<template>
  <div class="flex min-h-screen flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gray-100">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <h2 v-else class="mt-6 text-center text-3xl semibold tracking-tight text-gray-900">Login to your account</h2>
    </div>
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div class="bg-white px-4 py-8 shadow sm:rounded-lg sm:px-10">
        <form class="space-y-6" @submit="validate">
        ...
        <div v-if="variant === 'REGISTER'">
          <div class="row-h gap-2 text-sm mt-6 px-2 text-gray-500">
            ...
            <div class="pointer text-sky-500" @click="toggleVariant">Login</div>
          </div>
        </div>
        <div v-else>
          <div class="row-h gap-2 text-sm mt-6 px-2 text-gray-500">
            <div>New to this website?</div>
            <div class="pointer text-sky-500" @click="toggleVariant">Register</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      variant: this.initialVariant, // Initialize the data property with the prop
    }
  },
  computed: {
    isLoggedIn: {
  ...
  methods: {
     ...
     * Changes the value of the `variant` data property.
     */
    toggleVariant() {
    ...
    validate(e) {
      ...
      const passwordValue = this.$refs.passwordField.value
      const passwordWrapper = document.getElementById('password-wrapper')

      if (this.variant === 'REGISTER') {
        if (emailRegex.test(emailValue)) {
          // remove errors from previous email validation (if they exist)
          const errorNode = this.$refs.emailField.parentNode.getElementsByClassName('error')
          if (errorNode.length) {
            errorNode[0].remove()
          }
          ...
          if (!errorNode.length) {
            this.$refs.emailField.parentNode.insertAdjacentHTML(
              'beforeEnd',
        ...
        if (passwordValue.length < 8) {
          const errorNode = this.$refs.passwordField.parentNode.getElementsByClassName('error')
          if (errorNode.length) {
            errorNode[0].remove()
          }
          this.$refs.passwordField.parentNode.insertAdjacentHTML(
            'beforeEnd',
        ...
        const isValid = emailRegex.test(emailValue) && passwordValue.length >= 8 && passwordValue.length <= 20
        if (isValid) {
          this.isLoggedIn = true
          this.$nextTick(() => {
            this.$router.push('/') // The updated value
          })
        } else {
          return false
        }
